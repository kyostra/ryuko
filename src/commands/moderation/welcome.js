...
class Welcome extends Command {
    ...
    async handle ({ client, msg }, responder) {
        const fullPrompt = [
            'Ryuko.Welcome Prompt',
            '```pl',
            '\n`This is the Ryuko Welcome Prompt, please respond with choice you want to do~`',
            '\n',
        ...
        if (welcomer.choice === 'set') {
            const setWelcome = await responder.format('emoji:info').dialog([{
                prompt: [
                    'Ryuko.Welcome Set Prompt',
                    '\n```pl',
                    '\n`Please type the message you want Ryuko to send when a new member joins the server`',
                    '\n',
                    '\nArgs:',
