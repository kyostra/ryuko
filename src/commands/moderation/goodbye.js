...
class Goodbye extends Command {
    ...
    async handle ({ client, msg }, responder) {
        const fullPrompt = [
            'Ryuko.Goodbye Prompt',
            '```pl',
            '\n`This is the Ryuko Goodbye Prompt, please respond with choice you want to do~`',
            '\n',
        ...
        if (goodbyer.choice === 'set') {
            const setGoodbye = await responder.format('emoji:info').dialog([{
                prompt: [
                    'Ryuko.Goodbye Set Prompt',
                    '\n```pl',
                    '\n`Please type the message you want Ryuko to send when a member leaves the server`',
                    '\n',
                    '\nArgs:',
